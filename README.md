...
4. Copie `.env.example` a **`.env.local`** en la carpeta **`SistemaCanogar`** (junto a `package.json`) y rellene URL, `anon` y `service_role` desde Supabase → **Project Settings → API**.
...
Eso indica que **Next no está leyendo tu proyecto de Supabase**. El archivo **`.env.local` debe estar en la misma carpeta que `package.json`** (la carpeta `SistemaCanogar`, no el escritorio ni un padre). Después de crear o editar `.env.local`, **reinicie** `npm run dev`.
